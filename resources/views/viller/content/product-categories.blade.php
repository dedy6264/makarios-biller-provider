...
<div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center mb-card-gap">
    <div>
        <h2 class="font-headline-xl text-headline-xl text-on-surface">Product Categories</h2>
        <p class="mt-1 font-body-md text-body-md text-on-surface-variant">Kelola pengelompokan produk, hierarki
            inventaris, dan klasifikasi kategori sistem.</p>
    </div>
    <button
...
<x-app-layout>
    ...
    <div
        ...
        <div
            ...
            <div class="relative w-full md:w-80">
                ...
                <input id="search-input" name="category-table-search" autocomplete="off"
                    ...
                    placeholder="Cari kategori produk..." type="text">
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="categories-table" class="w-full text-left border-collapse">
                ...
                <tbody id="categories-tbody" class="bg-white">
                    <tr id="loading-row">
                        <td colspan="5"
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-category">
        <div class="absolute inset-0 bg-inverse-surface/40 backdrop-blur-sm" id="backdrop-category"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="glass-panel w-full max-w-[500px] rounded-[24px] shadow-[0px_10px_40px_rgba(0,0,0,0.10)] border border-white/40 overflow-hidden flex flex-col"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-outline-variant/50 bg-white/60">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/10 text-primary"
                            ...
                            <span class="material-symbols-outlined text-[20px]" id="modal-icon">folder_open</span>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div id="category-alert" class="hidden px-4 py-3 mb-5 rounded-xl text-body-sm font-body-sm"></div>

                    <div class="flex flex-col gap-1.5">
                        <label class="ml-1 font-label-caps text-label-caps text-on-surface-variant"
                            for="product_category_name">
                            Product Category Name <span class="text-error">*</span>
                        </label>
                        <div class="relative">
                            <span
                                class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-[20px] text-on-surface-variant">label</span>
                            <input id="product_category_name" type="text"
                                ...
                                placeholder="Masukkan nama kategori produk" autofocus>
                        </div>
                        <p id="category-name-error" class="hidden ml-1 text-xs text-error"></p>
                    </div>
                </div>
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-delete-category">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-2 font-headline-lg text-headline-lg text-on-surface">Hapus Kategori Produk?</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ========== Notification Toast Core Engine ==========
                ...
                document.getElementById('categories-tbody').innerHTML = `
                    <tr id="loading-row">
                        <td colspan="5" class="px-6 py-12 text-center text-on-surface-variant font-body-sm text-body-sm">
                            <div class="flex items-center justify-center gap-3">
                                <span class="material-symbols-outlined animate-spin text-primary">progress_activity</span>Memuat data...
                            </div>
                        </td>
                    </tr>`;

                fetch('{{ route('product_categories.getAll') }}', {
                    method: 'POST',
                    headers: {
                ...
                .catch(() => {
                    document.getElementById('categories-tbody').innerHTML = `
                        <tr><td colspan="5" class="px-6 py-10 text-center text-error font-body-sm">
                            <span class="material-symbols-outlined text-[32px] block mb-2">cloud_off</span>Gagal memuat data / Anda tidak memiliki otorisasi akses.
                        </td></tr>`;
                });
            }
            ...
            function renderTable() {
                ...
                const filtered = allCategories.filter(c => (c.product_category_name || '').toLowerCase().includes(searchVal));
                ...
                if (pageData.length === 0) {
                    tbody.innerHTML = `
                        <tr><td colspan="5" class="px-6 py-12 text-center text-on-surface-variant font-body-sm">
                            <span class="material-symbols-outlined text-[40px] block mb-2 opacity-40">search_off</span>Tidak ada data kategori ditemukan.
                        </td></tr>`;
                } else {
                    tbody.innerHTML = pageData.map((c, i) => {
                        ...
                        return `
                            <tr class="table-row-hover border-b border-surface-container group ${bg}">
                                ...
                                <td class="px-6 py-4 font-semibold font-data-tabular text-data-tabular text-on-surface">${c.product_category_name || '-'}</td>
                                <td class="px-6 py-4 font-body-sm text-body-sm text-on-surface-variant">${formatDate(c.created_at)}</td>
                                <td class="px-6 py-4 font-body-sm text-body-sm text-on-surface-variant">${formatDate(c.updated_at)}</td>
                                <td class="px-6 py-4">
                                    <div class="flex justify-center gap-2">
                                        <button onclick="openFormModal(${c.id})" class="p-2 transition-all rounded-lg text-on-surface-variant hover:text-primary hover:bg-primary/10 hover:scale-110" title="Edit">
                                            <span class="material-symbols-outlined text-[20px]">edit</span>
                                        </button>
                                        <button onclick="openDeleteModal(${c.id}, '${(c.product_category_name||'').replace(/'/g, "\\'")}')" class="p-2 transition-all rounded-lg text-on-surface-variant hover:text-error hover:bg-error-container hover:scale-110" title="Hapus">
                ...
                document.getElementById('pagination-info').textContent = total === 0 ? 'Tidak ada data' : `Menampilkan ${start+1}–${Math.min(start + perPage, total)} dari ${total} kategori`;
            ...
            // ========== Form Processing Actions (Store / Update) ==========
            document.getElementById('btn-add-category').addEventListener('click', () => {
                ...
                document.getElementById('modal-title').textContent = 'Add Product Category';
                document.getElementById('modal-icon').textContent = 'folder_open';
            ...
            window.openFormModal = (id) => {
                ...
                if (!target) return;
                isEditMode = true;
                document.getElementById('category-id').value = target.id;
                document.getElementById('product_category_name').value = target.product_category_name || '';
                document.getElementById('category-name-error').classList.add('hidden');
                document.getElementById('category-alert').classList.add('hidden');

                document.getElementById('modal-title').textContent = 'Edit Product Category';
            ...
            ['btn-close-modal', 'btn-cancel-modal', 'backdrop-category'].forEach(id => {
                document.getElementById(id).addEventListener('click', () => toggleModal('modal-category', false));
            });

            document.getElementById('btn-submit-modal').addEventListener('click', () => {
                const id = document.getElementById('category-id').value;
                const nameInput = document.getElementById('product_category_name').value.trim();
                const errEl = document.getElementById('category-name-error');
                ...
                const endpoint = isEditMode ? '{{ route('product_categories.update') }}' : '{{ route('product_categories.store') }}';
                const payload = isEditMode ? { id: id, product_category_name: nameInput } : { product_category_name: nameInput };
                ...
                .then(data => {
                    if (data.success !== false) {
                        toggleModal('modal-category', false);
                        showToast(data.message || 'Kategori produk berhasil disimpan!', 'success');
                        fetchProductCategories();
                    } else {
                        const alertEl = document.getElementById('category-alert');
                        alertEl.textContent = data.message || 'Terjadi kendala saat validasi entitas data.';
                ...
                .catch(() => {
                    const alertEl = document.getElementById('category-alert');
                    alertEl.textContent = 'Gagal melakukan sinkronisasi dengan endpoint cluster API.';
                    alertEl.classList.remove('hidden');
                })
            ...
            window.openDeleteModal = (id, name) => {
                deleteTargetId = id;
                document.getElementById('delete-confirm-text').textContent = `Kategori produk "${name}" akan dihapus permanen dari sistem database.`;
                toggleModal('modal-delete-category', true);
            };

            document.getElementById('btn-cancel-delete').addEventListener('click', () => {
                deleteTargetId = null;
                toggleModal('modal-delete-category', false);
            });
            ...
            document.getElementById('btn-confirm-delete').addEventListener('click', () => {
                ...
                fetch('{{ route('product_categories.destroy') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Content-Type': 'application/json',
                ...
                .then(data => {
                    toggleModal('modal-delete-category', false);
                    if (data.success !== false) {
                        showToast(data.message || 'Kategori produk berhasil dimusnahkan.', 'success');
                        fetchProductCategories();
                    } else {
                        showToast(data.message || 'Gagal mengeksekusi penghapusan data.', 'error');
                    }
                })
                .catch(() => {
                    toggleModal('modal-delete-category', false);
                    showToast('Terjadi kegagalan komunikasi transmisi data.', 'error');
                })
                .finally(() => {
                    deleteTargetId = null;
            ...
            // ========== Search Context Listeners ==========
            document.getElementById('search-input').addEventListener('input', () => {
                currentPage = 1;
                renderTable();
            });
            document.getElementById('btn-refresh').addEventListener('click', fetchProductCategories);
