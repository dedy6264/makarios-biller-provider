...
<div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center mb-card-gap">
    <div>
        <h2 class="font-headline-xl text-headline-xl text-on-surface">Product Types</h2>
        <p class="mt-1 font-body-md text-body-md text-on-surface-variant">Kelola klasifikasi, jenis kategori, dan tipe
            produk yang tersedia di sistem.</p>
    </div>
    <button
...
<x-app-layout>
    ...
    <div
        ...
        <div
            ...
            <div class="relative w-full md:w-80">
                ...
                <input id="search-input" name="type-table-search" autocomplete="off"
                    ...
                    placeholder="Cari tipe produk..." type="text">
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="types-table" class="w-full text-left border-collapse">
                <thead>
                ...
                <tbody id="types-tbody" class="bg-white">
                    <tr id="loading-row">
                        <td colspan="5"
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-type">
        <div class="absolute inset-0 bg-inverse-surface/40 backdrop-blur-sm" id="backdrop-type"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="glass-panel w-full max-w-[500px] rounded-[24px] shadow-[0px_10px_40px_rgba(0,0,0,0.10)] border border-white/40 overflow-hidden flex flex-col"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-outline-variant/50 bg-white/60">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/10 text-primary"
                            ...
                            <span class="material-symbols-outlined text-[20px]" id="modal-icon">category</span>
                        </div>
                        <h3 class="font-headline-lg text-headline-lg text-on-surface" id="modal-title">Add Product Type
                        </h3>
                    </div>
                    <button
                ...
                <div class="p-6">
                    ...
                    <div id="type-alert" class="hidden px-4 py-3 mb-5 rounded-xl text-body-sm font-body-sm"></div>

                    <div class="flex flex-col gap-1.5">
                        <label class="ml-1 font-label-caps text-label-caps text-on-surface-variant"
                            for="product_type_name">
                            Product Type Name <span class="text-error">*</span>
                        </label>
                        <div class="relative">
                            <span
                                class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-[20px] text-on-surface-variant">layers</span>
                            <input id="product_type_name" type="text"
                                ...
                                placeholder="Masukkan nama tipe produk" autofocus>
                        </div>
                        <p id="type-name-error" class="hidden ml-1 text-xs text-error"></p>
                    </div>
                </div>
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-delete-type">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-2 font-headline-lg text-headline-lg text-on-surface">Hapus Tipe Produk?</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function fetchProductTypes() {
                ...
                fetch('{{ route('product_types.getAll') }}', {
                    method: 'POST',
                    headers: {
                ...
                .catch(() => {
                    document.getElementById('types-tbody').innerHTML = `
                        <tr><td colspan="5" class="px-6 py-10 text-center text-error font-body-sm">
                            <span class="material-symbols-outlined text-[32px] block mb-2">cloud_off</span>Gagal memuat data / Anda tidak memiliki akses.
                        </td></tr>`;
                });
            }
            ...
            function renderTable() {
                ...
                const filtered = allTypes.filter(t => (t.product_type_name || '').toLowerCase().includes(searchVal));
                ...
                if (pageData.length === 0) {
                    tbody.innerHTML = `
                        <tr><td colspan="5" class="px-6 py-12 text-center text-on-surface-variant font-body-sm">
                            <span class="material-symbols-outlined text-[40px] block mb-2 opacity-40">search_off</span>Tidak ada data tipe produk ditemukan.
                        </td></tr>`;
                } else {
                    tbody.innerHTML = pageData.map((t, i) => {
                        ...
                        return `
                            <tr class="table-row-hover border-b border-surface-container group ${bg}">
                                ...
                                <td class="px-6 py-4 font-semibold font-data-tabular text-data-tabular text-on-surface">${t.product_type_name || '-'}</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center gap-2">
                                        <button onclick="openFormModal(${t.id})" class="p-2 transition-all rounded-lg text-on-surface-variant hover:text-primary hover:bg-primary/10 hover:scale-110" title="Edit">
                                            <span class="material-symbols-outlined text-[20px]">edit</span>
                                        </button>
                                        <button onclick="openDeleteModal(${t.id}, '${(t.product_type_name||'').replace(/'/g, "\\'")}')" class="p-2 transition-all rounded-lg text-on-surface-variant hover:text-error hover:bg-error-container hover:scale-110" title="Hapus">
                ...
                document.getElementById('pagination-info').textContent = total === 0 ? 'Tidak ada data' : `Menampilkan ${start+1}–${Math.min(start + perPage, total)} dari ${total} tipe produk`;
            ...
            // ========== Form Management (Create / Update) ==========
            document.getElementById('btn-add-type').addEventListener('click', () => {
                isEditMode = false;
                document.getElementById('type-id').value = '';
                document.getElementById('product_type_name').value = '';
                document.getElementById('type-name-error').classList.add('hidden');
                document.getElementById('type-alert').classList.add('hidden');
                
                document.getElementById('modal-title').textContent = 'Add Product Type';
                document.getElementById('modal-icon').textContent = 'category';
            ...
            window.openFormModal = (id) => {
                ...
                if (!target) return;
                isEditMode = true;
                document.getElementById('type-id').value = target.id;
                document.getElementById('product_type_name').value = target.product_type_name || '';
                document.getElementById('type-name-error').classList.add('hidden');
                document.getElementById('type-alert').classList.add('hidden');

                document.getElementById('modal-title').textContent = 'Edit Product Type';
            ...
            ['btn-close-modal', 'btn-cancel-modal', 'backdrop-type'].forEach(id => {
                document.getElementById(id).addEventListener('click', () => toggleModal('modal-type', false));
            });

            document.getElementById('btn-submit-modal').addEventListener('click', () => {
                const id = document.getElementById('type-id').value;
                const nameInput = document.getElementById('product_type_name').value.trim();
                const errEl = document.getElementById('type-name-error');
                
                if (!nameInput) {
                    errEl.textContent = 'Nama tipe produk wajib diisi.';
                    errEl.classList.remove('hidden');
                    return;
                }
                errEl.classList.add('hidden');

                const endpoint = isEditMode ? '{{ route('product_types.update') }}' : '{{ route('product_types.store') }}';
                const payload = isEditMode ? { id: id, product_type_name: nameInput } : { product_type_name: nameInput };
                ...
                .then(data => {
                    if (data.success !== false) {
                        toggleModal('modal-type', false);
                        showToast(data.message || 'Data tipe produk berhasil disimpan!', 'success');
                        fetchProductTypes();
                    } else {
                        const alertEl = document.getElementById('type-alert');
                        alertEl.textContent = data.message || 'Terjadi kesalahan penanganan data.';
                ...
                .catch(() => {
                    const alertEl = document.getElementById('type-alert');
                    alertEl.textContent = 'Gagal terhubung ke cluster API server.';
                    alertEl.classList.remove('hidden');
                })
            ...
            window.openDeleteModal = (id, name) => {
                deleteTargetId = id;
                document.getElementById('delete-confirm-text').textContent = `Tipe produk "${name}" akan dihapus secara permanen.`;
                toggleModal('modal-delete-type', true);
            };

            document.getElementById('btn-cancel-delete').addEventListener('click', () => {
                deleteTargetId = null;
                toggleModal('modal-delete-type', false);
            });
            ...
            document.getElementById('btn-confirm-delete').addEventListener('click', () => {
                ...
                fetch('{{ route('product_types.destroy') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Content-Type': 'application/json',
                ...
                .then(data => {
                    toggleModal('modal-delete-type', false);
                    if (data.success !== false) {
                        showToast(data.message || 'Tipe produk berhasil dihapus.', 'success');
                        fetchProductTypes();
                    } else {
                        showToast(data.message || 'Gagal menghapus entitas produk.', 'error');
                    }
                })
                .catch(() => {
                    toggleModal('modal-delete-type', false);
            ...
            document.getElementById('btn-refresh').addEventListener('click', fetchProductTypes);
