...
<div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center mb-card-gap">
    <div>
        <h2 class="font-headline-xl text-headline-xl text-on-surface">Accounts</h2>
        <p class="mt-1 font-body-md text-body-md text-on-surface-variant">Kelola daftar akun sistem, pemetaan data
            keuangan, dan pengaturan ledger akun.</p>
    </div>
    <button
...
<x-app-layout>
    ...
    <div
        ...
        <div
            ...
            <div class="relative w-full md:w-80">
                ...
                <input id="search-input" name="account-table-search" autocomplete="off"
                    ...
                    placeholder="Cari nama akun..." type="text">
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="accounts-table" class="w-full text-left border-collapse">
                <thead>
                ...
                <tbody id="accounts-tbody" class="bg-white">
                    <tr id="loading-row">
                        <td colspan="5"
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-account">
        <div class="absolute inset-0 bg-inverse-surface/40 backdrop-blur-sm" id="backdrop-account"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="glass-panel w-full max-w-[500px] rounded-[24px] shadow-[0px_10px_40px_rgba(0,0,0,0.10)] border border-white/40 overflow-hidden flex flex-col"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/10 text-primary"
                            ...
                            <span class="material-symbols-outlined text-[20px]"
                                id="modal-icon">account_balance_wallet</span>
                        </div>
                        <h3 class="font-headline-lg text-headline-lg text-on-surface" id="modal-title">Add Account</h3>
                    </div>
                    <button
                ...
                <div class="p-6">
                    ...
                    <div id="account-alert" class="hidden px-4 py-3 mb-5 rounded-xl text-body-sm font-body-sm"></div>

                    <div class="flex flex-col gap-1.5">
                        <label class="ml-1 font-label-caps text-label-caps text-on-surface-variant" for="account_name">
                        ...
                        <div class="relative">
                            ...
                            <input id="account_name" type="text"
                                ...
                                placeholder="Masukkan nama akun baru" autofocus>
                        </div>
                        <p id="account-name-error" class="hidden ml-1 text-xs text-error"></p>
                    </div>
                </div>
    ...
    <div class="fixed inset-0 z-50 hidden" id="modal-delete-account">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-2 font-headline-lg text-headline-lg text-on-surface">Hapus Akun?</h3>
                    <p id="delete-confirm-text" class="font-body-md text-body-md text-on-surface-variant">Tindakan ini
                        akan menghapus akun secara permanen.</p>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ========== Notification Toast Logic Drivers ==========
            ...
            // Gunakan toggleModal agar konsisten dengan layout utama
            ...
            // ========== Server AJAX Payload Pulls & Table Renders ==========
                ...
                document.getElementById('accounts-tbody').innerHTML = `
                    <tr id="loading-row">
                        <td colspan="5" class="px-6 py-12 text-center text-on-surface-variant font-body-sm text-body-sm">
                            <div class="flex items-center justify-center gap-3">
                                <span class="material-symbols-outlined animate-spin text-primary">progress_activity</span>Memuat data akun...
                            </div>
                        </td>
                    </tr>`;

                fetch('{{ route('accounts.getAll') }}', {
                    method: 'POST',
                    headers: {
                ...
                .catch(() => {
                    document.getElementById('accounts-tbody').innerHTML = `
                        <tr><td colspan="5" class="px-6 py-10 text-center text-error font-body-sm">
                            <span class="material-symbols-outlined text-[32px] block mb-2">cloud_off</span>Gagal memuat repositori akun / Anda tidak memiliki izin.
                        </td></tr>`;
                });
            }
            ...
            function renderTable() {
                ...
                const filtered = allAccounts.filter(a => (a.account_name || '').toLowerCase().includes(searchVal));
                ...
                if (pageData.length === 0) {
                    tbody.innerHTML = `
                        <tr><td colspan="5" class="px-6 py-12 text-center text-on-surface-variant font-body-sm">
                            <span class="material-symbols-outlined text-[40px] block mb-2 opacity-40">search_off</span>Tidak ada data akun ditemukan.
                        </td></tr>`;
                } else {
                    tbody.innerHTML = pageData.map((p, i) => {
                        ...
                        return `
                            <tr class="table-row-hover border-b border-surface-container group ${bg}">
                                ...
                                <td class="px-6 py-4 font-semibold font-data-tabular text-data-tabular text-on-surface">${p.account_name || '-'}</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <button onclick="openDeleteModal(${p.id}, '${(p.account_name||'').replace(/'/g, "\\'")}')" class="p-2 transition-all rounded-lg text-on-surface-variant hover:text-error hover:bg-error-container hover:scale-110" title="Hapus">
                ...
                document.getElementById('pagination-info').textContent = total === 0 ? 'Tidak ada data' : `Menampilkan ${start+1}–${Math.min(start + perPage, total)} dari ${total} akun`;
            ...
            // ========== Form Context Operations (Store / Update) ==========
            document.getElementById('btn-add-account').addEventListener('click', () => {
                ...
                document.getElementById('modal-icon').textContent = 'account_balance_wallet';
            ...
            window.openFormModal = (id) => {
                ...
                if (!target) return;
                isEditMode = true;
                document.getElementById('account-id').value = target.id;
                document.getElementById('account_name').value = target.account_name || '';
                document.getElementById('account-name-error').classList.add('hidden');
                document.getElementById('account-alert').classList.add('hidden');
            ...
            ['btn-close-modal', 'btn-cancel-modal', 'backdrop-account'].forEach(id => {
                document.getElementById(id).addEventListener('click', () => toggleModal('modal-account', false));
            });

            document.getElementById('btn-submit-modal').addEventListener('click', () => {
                const id = document.getElementById('account-id').value;
                const nameInput = document.getElementById('account_name').value.trim();
                const errEl = document.getElementById('account-name-error');
                
                if (!nameInput) {
                    errEl.textContent = 'Nama akun wajib diisi.';
                    errEl.classList.remove('hidden');
                    return;
                }
                errEl.classList.add('hidden');

                const endpoint = isEditMode ? '{{ route('accounts.update') }}' : '{{ route('accounts.store') }}';
                const payload = isEditMode ? { id: id, account_name: nameInput } : { account_name: nameInput };
                ...
                .then(data => {
                    if (data.success !== false) {
                        ...
                        showToast(data.message || 'Akun berhasil disimpan!', 'success');
                        fetchAccounts();
                    } else {
                        const alertEl = document.getElementById('account-alert');
                        alertEl.textContent = data.message || 'Terjadi kesalahan saat validasi data.';
            ...
            // ========== Delete Routine Core Execution Flow ==========
            window.openDeleteModal = (id, name) => {
                deleteTargetId = id;
                document.getElementById('delete-confirm-text').textContent = `Akun "${name}" akan dihapus secara permanen dari basis data sistem.`;
                toggleModal('modal-delete-account', true);
            };

            document.getElementById('btn-cancel-delete').addEventListener('click', () => {
                deleteTargetId = null;
                toggleModal('modal-delete-account', false);
            });
            ...
            document.getElementById('btn-confirm-delete').addEventListener('click', () => {
                ...
                .then(data => {
                    ...
                    if (data.success !== false) {
                        ...
                        showToast(data.message || 'Gagal menghapus entitas akun.', 'error');
                    }
                })
                .catch(() => {
                    toggleModal('modal-delete-account', false);
