                ...
                <table id="tabeldata" class="mdl-data-table">
                  <thead>
                    <tr >
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Role Code</th>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Role' : 'Add Role' }}</h5>
        </div>
                      ...
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          <div class="mb-3">
                            ...
                            <input type="text" v-model="form.role_name" class="form-control" id="role_name" placeholder="Role Name" aria-label="Role Name" aria-describedby="email-addon">
                          </div>
                          <div class="mb-3">
                            <label for="role_code" class="form-label">Role Code</label>
                            <input type="text" v-model="form.role_code" class="form-control" id="role_code" placeholder="Role Code" aria-label="Role Code" aria-describedby="email-addon">
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateRole() : storeRole()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
      ...
      createApp({
        setup() {
          ...
          const closeModal = () => {
            ...
            form.value.role_code= "";
            myModal.hide();
            isEditMode.value = false;
          };
          const refreshDataRole = () => {
                    ...
                    columns: [
                        ...
                        { data: 'id',render:function(data,type){
                         return `
                            ...
                            <button class="btn btn-sm btn-outline-primary" onclick="deleteRole(${data})">
                                Delete
                            </button>`} },
                        { data: 'role_name' },
                        { data: 'role_code' },
          ...
          window.updateModal = (idRole) => {
              for(let a=0;a<mainData.value.length;a++){
                if (mainData.value[a].id==idRole){
                  form.value.id = idRole;
                  form.value.role_name = mainData.value[a].role_name;
                  form.value.role_code= mainData.value[a].role_code;
                  isEditMode.value = true;
                  myModal.show();
                }
              };
          };
          window.deleteRole = (idRole) => {
            console.log("Delete role with ID:", idRole);
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('roles.destroy') }}', { id: idRole })
                  .then(response => {
                    refreshDataRole();
                  })
                  .catch(error => {
          ...
          const updateRole=()=>{
            nextTick(()=>{
                ...
                axios.post('{{ route('roles.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshDataRole();
          ...
          return { 
            refreshDataRole,isModalOpen,
            openModal,
            closeModal,updateModal,form,updateRole,storeRole,isEditMode,
          };
        }
      }).mount('#app');
    </script>
