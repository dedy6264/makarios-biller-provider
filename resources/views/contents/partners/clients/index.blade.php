    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Client' : 'Add Client' }}</h5>
        </div>
                      ...
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          <div class="mb-3">
                            ...
                            <input type="text" v-model="form.client_name" class="form-control" id="client_name" placeholder="Client Name" aria-label="Client Name" aria-describedby="email-addon" autofocus>
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateClient() : storeClient()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
        ...
        setup() {
                    ...
                    columns: [
                        ...
                        { data: 'id',render:function(data,type){
                         return `
                            ...
                            <button class="btn btn-sm btn-outline-primary" onclick="deleteClient(${data})">
                                Delete
                            </button>`} },
                        { data: 'client_name' },
                        { data: 'updated_at' },
          ...
          const updateClient=()=>{
            nextTick(()=>{
                ...
                axios.post('{{ route('clients.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshDataClient();
