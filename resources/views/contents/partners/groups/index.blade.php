    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Group' : 'Add Group' }}</h5>
        </div>
                      ...
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          ...
                          <div class="mb-3">
                            ...
                            <input type="text" v-model="form.group_name" class="form-control" id="group_name" placeholder="Group Name" aria-label="Group Name" aria-describedby="email-addon">
                          </div>
                          {{-- <div class="mb-3">
                            <label for="role_code" class="form-label">Group Code</label>
                            <input type="text" v-model="form.role_code" class="form-control" id="role_code" placeholder="Group Code" aria-label="Group Code" aria-describedby="email-addon">
                          </div> --}}
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateGroup() : storeGroup()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
      ...
      createApp({
        setup() {
          ...
          window.deleteGroup = (idGroup) => {
            ...
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('groups.destroy') }}', { id: idGroup })
                  .then(response => {
                    refreshData();
                  })
                  .catch(error => {
          ...
          const storeGroup=()=>{
            ...
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('groups.store') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshData();
          ...
          const updateGroup=()=>{
            ...
            nextTick(()=>{
                axios.post('{{ route('groups.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshData();
          ...
          return { 
            ...
            openModal,
            closeModal,updateModal,form,updateGroup,storeGroup,isEditMode,clients
          };
        }
      }).mount('#app');
    </script>
