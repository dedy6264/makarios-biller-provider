    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateUserRole() : storeUserRole()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <script>
      ...
      createApp({
        setup() {
                    ...
                    columns: [
                        ...
                        { data: 'id',render:function(data,type){
                         return `
                            ...
                            <button class="btn btn-sm btn-outline-primary" onclick="deleteUserRole(${data})">
                                Delete
                            </button>`} },
                        { data: 'user_name' },
                        { data: 'role_name' },
          ...
          window.deleteUserRole = (idUserRole) => {
            console.log("Delete role with ID:", idUserRole);
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('user_roles.destroy') }}', { id: idUserRole })
                  .then(response => {
                    refreshData();
                  })
                  .catch(error => {
          ...
          const storeUserRole=()=>{
            ...
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('user_roles.store') }}', form.value)
                  .then(response => {
                    // console.log("UserRole created:", response.data);
                    isEditMode.value = false;
                    closeModal();
                    refreshData();
                  })
