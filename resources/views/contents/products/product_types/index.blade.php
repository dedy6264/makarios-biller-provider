            ...
            <div class="p-3 pb-0 card-header">
              <div class="row">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Product Types</h6>
                </div>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Product Type' : 'Add Product Type' }}</h5>
        </div>
                      ...
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          <div class="mb-3">
                            ...
                            <input type="text" v-model="form.product_type_name" class="form-control" id="product_type_name" placeholder="Product Type Name" aria-label="Product Type Name" aria-describedby="email-addon" autofocus>
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateProductType() : storeProductType()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <script>
      ...
      createApp({
        setup() {
          ...
          const refreshDataProductType = () => {
            ...
            nextTick(() => {
              // if ($.fn.DataTable) {
                $('#tabeldata').DataTable({
                    ajax: {
                      url: '{{ route('product_types.getAll') }}',
                      method: 'POST',
                      headers: {
          ...
          window.updateModal = (idProductType) => {
              for(let a=0;a<mainData.value.length;a++){
                if (mainData.value[a].id==idProductType){
                  form.value.id = idProductType;
          ...
          window.deleteProductType = (idProductType) => {
            console.log("Delete client with ID:", idProductType);
            nextTick(()=>{
                // Create new client
               axios.post('{{ route('product_types.destroy') }}', { id: idProductType })
                  .then(response => {
                    refreshDataProductType();
                  })
                  .catch(error => {
          ...
          const storeProductType=()=>{
            nextTick(()=>{
                // Create new client
               axios.post('{{ route('product_types.store') }}', form.value)
                  .then(response => {
          ...
          const updateProductType=()=>{
            nextTick(()=>{
                ...
                axios.post('{{ route('product_types.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshDataProductType();
