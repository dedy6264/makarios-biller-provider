            ...
            <div class="p-3 pb-0 card-header">
              <div class="row">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Product References</h6>
                </div>
                ...
                <table id="tabeldata" class="mdl-data-table">
                  <thead>
                    <tr >
                      ...
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Product Reference Name</th>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Product Reference' : 'Add Product Reference' }}</h5>
        </div>
                  ...
                  <div class="card-body">
                    <form role="form text-left">
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          <div class="mb-3">
                            <label for="product_reference_name" class="form-label">Product Reference Name</label>
                            <input type="text" v-model="form.product_reference_name" class="form-control" id="product_reference_name" placeholder="Product Reference Name" aria-label="Product Reference Name" aria-describedby="email-addon" autofocus>
                          </div>
                          <div class="mb-3">
                            <label for="product_reference_code" class="form-label">Product Reference Code</label>
                            <input type="text" v-model="form.product_reference_code" class="form-control" id="product_reference_code" placeholder="Product Reference Code" aria-label="Product Reference Code" aria-describedby="email-addon" autofocus>
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateProductReference() : storeProductReference()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <script>
      ...
      createApp({
        setup() {
          ...
          const refreshDataProductReference = () => {
            ...
            nextTick(() => {
              // if ($.fn.DataTable) {
                $('#tabeldata').DataTable({
                    ajax: {
                      url: '{{ route('product_references.getAll') }}',
                      method: 'POST',
                      headers: {
          ...
          window.updateModal = (idProductReference) => {
              for(let a=0;a<mainData.value.length;a++){
                if (mainData.value[a].id==idProductReference){
                  form.value.id = idProductReference;
          ...
          window.deleteProductType = (idProductReference) => {
            console.log("Delete Reference with ID:", idProductReference);
            nextTick(()=>{
                // Create new Reference
               axios.post('{{ route('product_references.destroy') }}', { id: idProductReference })
                  .then(response => {
                    refreshDataProductReference();
                  })
                  .catch(error => {
          ...
          const storeProductReference=()=>{
            nextTick(()=>{
                // Create new Reference
               axios.post('{{ route('product_references.store') }}', form.value)
                  .then(response => {
          ...
          const updateProductReference=()=>{
            nextTick(()=>{
                // Update existing Reference
                axios.post('{{ route('product_references.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshDataProductReference();
