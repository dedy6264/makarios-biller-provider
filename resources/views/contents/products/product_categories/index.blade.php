    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">@{{ isEditMode ? 'Edit Product Category' : 'Add Product Category' }}</h5>
        </div>
                      ...
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          <div class="mb-3">
                            ...
                            <input type="text" v-model="form.product_category_name" class="form-control" id="product_category_name" placeholder="Product Category Name" aria-label="Product Category Name" aria-describedby="email-addon" autofocus>
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateProductCategory() : storeProductCategory()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <script>
      ...
      createApp({
        setup() {
          ...
          const refreshDataProductCategory = () => {
            ...
            nextTick(() => {
              // if ($.fn.DataTable) {
                $('#tabeldata').DataTable({
                    ajax: {
                      url: '{{ route('product_categories.getAll') }}',
                      method: 'POST',
                      headers: {
          ...
          window.updateModal = (idProductCategory) => {
              for(let a=0;a<mainData.value.length;a++){
                if (mainData.value[a].id==idProductCategory){
                  form.value.id = idProductCategory;
          ...
          window.deleteProductType = (idProductCategory) => {
            console.log("Delete Category with ID:", idProductCategory);
            nextTick(()=>{
                // Create new Category
               axios.post('{{ route('product_categories.destroy') }}', { id: idProductCategory })
                  .then(response => {
                    refreshDataProductCategory();
                  })
                  .catch(error => {
          ...
          const storeProductCategory=()=>{
            nextTick(()=>{
                // Create new Category
               axios.post('{{ route('product_categories.store') }}', form.value)
                  .then(response => {
          ...
          const updateProductCategory=()=>{
            nextTick(()=>{
                ...
                axios.post('{{ route('product_categories.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshDataProductCategory();
