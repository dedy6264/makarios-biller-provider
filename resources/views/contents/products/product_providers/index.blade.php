            ...
            <div class="p-3 pb-0 card-header">
              <div class="row">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Product Providers</h6>
                </div>
                ...
                <table id="tabeldata" class="mdl-data-table">
                  <thead>
                    <tr >
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Provider Name</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Product Provider Price</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Product Provider Admin Fee</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Product Provider Merchant Fee</th>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
                    ...
                    <form role="form text-left">
                      {{-- <div class="row"> --}}
                        {{-- <div class="col-lg-6"> --}}
                          ...
                          <div class="mb-3">
                            <label for="product_provider_name" class="form-label">Product Name</label>
                            <input type="text" v-model="form.product_provider_name" class="form-control" id="product_provider_name" placeholder="Product Name" aria-label="Product Name" aria-describedby="email-addon">
                          </div>
                          <div class="mb-3">
                            <label for="product_provider_code" class="form-label">Product Code</label>
                            <input type="text" v-model="form.product_provider_code" class="form-control" id="product_provider_code" placeholder="Product Code" aria-label="Product Code" aria-describedby="email-addon">
                          </div>
                          <div class="mb-3">
                            <label for="product_provider_price" class="form-label">Product Price</label>
                            <input type="currency" v-model="form.product_provider_price" class="form-control" id="product_provider_price" placeholder="Product Price" aria-label="Product Price" aria-describedby="email-addon">
                          </div>
                          <div class="mb-3">
                            <label for="product_provider_admin_fee" class="form-label">Product Admin Fee</label>
                            <input type="currency" v-model="form.product_provider_admin_fee" class="form-control" id="product_provider_admin_fee" placeholder="Product Admin Fee" aria-label="Product Admin Fee" aria-describedby="email-addon">
                          </div>
                          <div class="mb-3">
                            <label for="product_provider_merchant_fee" class="form-label">Product Merchant Fee</label>
                            <input type="currency" v-model="form.product_provider_merchant_fee" class="form-control" id="product_provider_merchant_fee" placeholder="Product Merchant Fee" aria-label="Product Admin Fee" aria-describedby="email-addon">
                          </div>
                        {{-- </div> --}}
                      {{-- </div> --}}
                    </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="isEditMode ? updateProductProvider() : storeProductProvider()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
      ...
      createApp({
        setup() {
          ...
          const refreshData = () => {
            ...
            nextTick(() => {
              // if ($.fn.DataTable) {
                $('#tabeldata').DataTable({
                    ajax: {
                      url: '{{ route('product_providers.getAll') }}',
                      method: 'POST',
                      headers: {
          ...
          window.deleteGroup = (idGroup) => {
            ...
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('product_providers.destroy') }}', { id: idGroup })
                  .then(response => {
                    refreshData();
                  })
                  .catch(error => {
          ...
          const storeProductProvider=()=>{
            ...
            nextTick(()=>{
                // Create new role
               axios.post('{{ route('product_providers.store') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshData();
          ...
          const updateProductProvider=()=>{
            ...
            nextTick(()=>{
                axios.post('{{ route('product_providers.update') }}', form.value)
                  .then(response => {
                    isEditMode.value = false;
                    closeModal();
                    refreshData();
          ...
          return { 
            ...
            openModal,
            closeModal,updateModal,form,updateProductProvider,storeProductProvider,isEditMode,providers,
          };
        }
      }).mount('#app');
    </script>
